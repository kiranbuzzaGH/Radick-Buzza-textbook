...
One can easily construct the \textit{empty set} using the condition $x \neq x$. Clearly there are no
elements of such a set. Denote this as $\{x \text{ } \epsilon \text{ } A: x \neq x \} = \varnothing$.
...
\epsilon \text{ } A$. This is actually a special case of the Axiom of Specification\footnote{Reformulate the Axiom of Pairing as $x \;
\epsilon \; B \text{ if and only if } S(x)$, where $S(x) = "x = a \text{ or } x = b"$, to more easily compare. Think more about this.}.
...
pairs of these and the singletons and so on. Note that not all of these sets are unique ($\{\varnothing, \{\varnothing\}\} = \varnothing$, for example).

Let us introduce new notation: if $S(x)$ is a condition on $x$, then denote the set by $\{x:S(x)\}$. For example
\begin{displaymath}
    \begin{aligned}
        \{x: x \; \epsilon\; A\} = A,\\
        \{x: x \neq x\} = \varnothing,\\
