""" Sort an array of n elements by Merge Sort with Insertion Sort on small arrays/lists."""

def merge_sort(A, k=10, p=0, q=None):
    ...
    if q is None:
        q = len(A) - 1
    if q - p <= k - 1:
        return insertion_sort(A, p, q)

    mid = (p + q) // 2
    merge_sort(A, k, p, mid)
    merge_sort(A, k, mid + 1, q)
    merge(A, p, mid, q)
...
def insertion_sort(A, p, q):
    ...
    Arguments:
    A -- a list
    p -- index of first element in the sublist/subarray
    q -- index of last element in the sublist
    """
    # Traverse the list from index p + 1 to q
